...
input int              PSPeriod        = 5;               // Price smoothing period
input enMaTypes        PSMaType        = ma_sma;          // Price smoothing method
extern int             BandsPeriod     = 20;              // Bands period
extern double          BandsDeviation  = 2;               // Bands deviation
extern bool            BandsDeviationSample = false;      // Bands deviation with sample correction?
extern double          BandsRisk       = 0.5;             // Bands risk
...
extern int             ArrowCodeUp     = 108;             // Arrow code up
extern int             ArrowCodeDn     = 108;             // Arrow code down
...
#define _mtfCall(_buff,_y) iCustom(NULL,TimeFrame,indicatorFileName,PERIOD_CURRENT,PSPeriod,PSMaType,BandsPeriod,BandsDeviation,BandsDeviationSample,BandsRisk,BandsMaType,Price,alertsOn,alertsOnCurrent,alertsMessage,alertsSound,alertsNotify,alertsEmail,soundFile,_buff,_y)
...
int OnInit()
{
   ...
   IndicatorBuffers(13);
         ...
         SetIndexBuffer(5,upa);      SetIndexStyle(5,arst,0,ArrowSize,clup); SetIndexArrow(5,ArrowCodeUp);
         SetIndexBuffer(6,dna);      SetIndexStyle(6,arst,0,ArrowSize,cldn); SetIndexArrow(6,ArrowCodeDn);
                 ...
                 const int&      spread[] )
{
   ...
   for(int i=limit; i>=0; i--)
   {
      double price = iCustomMa(PSMaType,getPrice(Price,open,close,high,low,i,rates_total),PSPeriod,i,0);
      double dev   = iDeviation(price,BandsPeriod,BandsDeviationSample,i);
               zero[i]  = iCustomMa(BandsMaType,price,BandsPeriod,i,1);
